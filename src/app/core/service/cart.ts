...
addToCart(product: Product) : void{
  ...
  this.cartItems.next(existing? items.map(f => f.product.id === product.id ? {...f, quantity: f.quantity+1 }: f):[...items, {product,quantity:1}]);
}
  removeFromCart(id: string): void {
    const updatedItems =
      this.cartItems.value.filter(item => item.product.id !== id);
